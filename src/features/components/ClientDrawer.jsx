...
import { ClientFirstFeature } from "../user/client/presentationala/ClientCard";
...
const clientTemporaryLeftDrawerData = [
  ...
  { name: "Projects", icon: <AssignmentTurnedInOutlinedIcon />,  url: "#Outbox" },
  { name: "Applications", icon: <WorkspacePremiumOutlinedIcon />, url: "#Home" },
  { name: "Support", icon: <HelpCenterOutlinedIcon /> ,  url: "#Home"},
  { name: "Settings", icon: <SettingsOutlinedIcon /> ,  url: "#Home"},
  { name: "Logout", icon: <LogoutOutlinedIcon sx={{ color: "red" }} /> , url: "#Home"},
];
...
const getList = () => (
  <div style={{ width: 250 }}>
    {clientTemporaryLeftDrawerData.map((item, index) => (
      ...
      <a style={{textDecoration:"none"}} href={item.url} ><ListItemText primary={item.name} /> </a>  
      </ListItem>
    ))}
  </div>
);
...
return (
  <Container >
    ...
    >
      ...
      <ClientFirstFeature/>
      <Link href="/client-profile" className="clientDrawerLink" sx={{textDecoration:"none"}}>View Profile</Link>
      </Box>
